...
* **Structural Errors:** 
    * PK duplicates or missing required columns set the stage status to `failed`. This informs the global orchestrator that the data is unsafe for the Contract stage.
* **Integrity Warnings:** 
    * Orphan records or temporal impossibilities (e.g., delivered before purchased) are logged as `warnings`. 
    * The stage may still return a `success` status if the underlying structure is sound, delegating the cleanup to the [`contract stage`](contract_stage.md).
* **Incomplete Context:** 
