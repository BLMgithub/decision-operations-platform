...
6.  **Memory Management:** Explicitly deletes DataFrames and triggers `gc.collect()` after each table export to prevent memory accumulation during large-scale aggregations.
